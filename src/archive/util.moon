...
export inventory = {
  inventoryItemTypeCache: {},
  getInventoryItemType: (inv, force) ->
    if not force and (cached = inventory.inventoryItemTypeCache[inv]) then return cached
    size = inv.size
    local itemType
    for i = 0, size
      itemStack = inv\getStack i
      if itemType and itemStack.item.type ~= itemType
        error('mixed inventories are unsupported')
      elseif itemStack and itemStack.item.type
